...
// Send email via Resend (recommended) or SendGrid
async function sendEmail(to: string, subject: string, html: string) {
  ...
  try {
    const response = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: "Moxie AI <calls@moxiepm.com>",
        to,
        subject,
        html,
      }),
...
export async function POST(request: NextRequest) {
  try {
    ...
    const teamPhone = process.env.TEAM_EMERGENCY_PHONE;
    const teamEmail = process.env.TEAM_EMAIL;
    
    if (!teamPhone && !teamEmail) {
      console.error("No team contact info configured");
    ...
    if (payload.emergency && teamPhone) {
...
${payload.recording_url ? `Recording: ${payload.recording_url}` : ""}`;
      ...
      await sendSMS(teamPhone, emergencyMessage);
      console.log("Emergency SMS sent to", teamPhone);
    }
    ...
    if (teamEmail) {
      ...
      const emailHtml = `
        ...
        <hr>
        <p><em>Sent by Moxie AI After-Hours System</em></p>
      `;

      await sendEmail(teamEmail, emailSubject, emailHtml);
      console.log("Email summary sent to", teamEmail);
    }

    // TODO: Store call in database for history
    // await storeCallLog(payload);
